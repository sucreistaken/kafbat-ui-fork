...
abstract class AbstractEmitter implements java.util.function.Consumer<FluxSink<TopicMessageEventDTO>> {
  ...
  protected void sendConsuming(FluxSink<TopicMessageEventDTO> sink, PolledRecords records) {
    sendConsuming(sink, records, records.count());
  }

  protected void sendConsuming(FluxSink<TopicMessageEventDTO> sink, PolledRecords records, int countedRecords) {
    messagesProcessing.sentConsumingInfo(sink, records, countedRecords);
  }
